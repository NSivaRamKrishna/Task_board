...
const Taskbar = ({ onTaskSelected }) => {
  ...
  const fetchTaskData = async () => {
    try {
      ...
      const res = await axios.get(`https://todo-backend-psi-nine.vercel.app/task/${id}`);
      if (res.status === 200) {
  ...
  const handleAddTask = async (task) => {
    const id = userData.userid;
    try {
      const res = await axios.post(`https://todo-backend-psi-nine.vercel.app/task/add/${id}`, { task });
      if (res.status === 200) {
  ...
  const handleDelete = () => {
    ...
    axios
      .post(`https://todo-backend-psi-nine.vercel.app/task/remove/${selectedTask.id}`)
      .then((res) => {
        if (res.status === 200) {
          setTempCounter((prevCount) => prevCount + 1);
        }
  ...
  const handleEditSubmit = (editedTask) => {
    axios.post(`https://todo-backend-psi-nine.vercel.app/task/update/${selectedTask.id}`,{task:editedTask})
    .then((res)=>{
      if(res.status === 200){
        setTempCounter((prevCount)=> prevCount + 1);
      }
