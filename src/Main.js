...
const Main = ({ taskInfo }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://todo-backend-psi-nine.vercel.app/todo/${taskInfo.id}`
      );
  ...
  const handleAddTodo = async (data) => {
    ...
    axios
      .post(`https://todo-backend-psi-nine.vercel.app/todo/add/${id}`, {
        content: data.name,
  ...
  const handleEditSubmit = (editedData) => {
    axios.post(`https://todo-backend-psi-nine.vercel.app/todo/update/${editedData.id}`,{
  ...
  const handleDelete = (itemId, index) => {
    axios
      .post(`https://todo-backend-psi-nine.vercel.app/todo/remove/${itemId}`)
      .then((res) => {
        if (res.status === 200) {
          fetchData();
        }
